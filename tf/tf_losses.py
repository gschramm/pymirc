...
from .tf_metrics import (
    ...
    IoU,
    ssim_3d,
    generalized_dice_coeff,
)


def weighted_binary_crossentropy(weights=[0.5, 1]):
...
def focal_loss(gamma=2.0, alpha=0.25, from_logits=False):
    """Focal loss
    ...
    def focal_loss_fixed(y_true, y_pred):
        weight_mask = 10 * (
            alpha * y_pred * (1 - y_true) + (1 - alpha) * (1 - y_pred) * y_true
        )
        return K.mean(
            weight_mask
            * K.binary_crossentropy(y_true, y_pred, from_logits=from_logits),
            axis=-1,
        )
