...
class ThreeAxisViewer:
    ...
    def __init__(
        ...
        if self.ndim:
            self.nframes = self.vols[0].shape[0]
        else:
            self.nframes = 1
        ...
        if not isinstance(self.imshow_kwargs, list):
            ...
            for i in range(self.n_vols):
                self.imshow_kwargs.append(tmp.copy())
        ...
        if not isinstance(self.oimshow_kwargs, list):
            ...
            self.oimshow_kwargs = []
            for i in range(self.n_vols):
                self.oimshow_kwargs.append(tmp.copy())
        ...
        if width == None:
            width = min(12, 24 / len(self.vols))
        fig_asp = (
            self.n_vols
            * max(self.shape[self.iy], self.shape[self.iz])
            / (2 * self.shape[self.ix] + self.shape[self.iy])
        )

        self.fig, self.ax = py.subplots(
            self.n_vols, 3, figsize=(width, width * fig_asp), squeeze=False
        )
        ...
        for i in range(self.n_vols):
            ...
            tmp.append(
                self.ax[i, 0].imshow(
                    im0, aspect=voxsize[1] / voxsize[0], **self.imshow_kwargs[i]
                )
            )
            tmp.append(
                self.ax[i, 1].imshow(
                    im1, aspect=voxsize[2] / voxsize[0], **self.imshow_kwargs[i]
                )
            )
            tmp.append(
                self.ax[i, 2].imshow(
                    im2, aspect=voxsize[2] / voxsize[1], **self.imshow_kwargs[i]
                )
            )
        ...
        if rowlabels is None:
            self.fig.subplots_adjust(
                left=0, right=0.95, bottom=0, top=0.97, wspace=0.01, hspace=0.01
            )
            ...
            self.fig.subplots_adjust(
                left=0.03, right=0.95, bottom=0, top=0.97, wspace=0.01, hspace=0.01
            )
        self.cb_ax = []
        ...
        for irow in range(self.n_vols):
            ...
            for icol in range(3):
                bbox = bboxes[icol]
                self.ax[irow, icol].set_position(
                    [bbox.x0, y0min, bbox.x1 - bbox.x0, bbox.y1 - bbox.y0]
                )
            ...
            self.cb_ax.append(
                inset_axes(
                    ...
                    bbox_transform=self.ax[irow, -1].transAxes,
                    borderpad=0,
                )
            )
        ...
        for i in range(self.n_vols):
            self.cb_ax[i].imshow(
                np.arange(128).reshape((128, 1)),
                aspect=0.2,
                origin="lower",
                cmap=self.imshow_kwargs[i]["cmap"],
            )
            self.cb_top_labels.append(
                self.cb_ax[i].text(
                    1.2,
                    1,
                    f'{self.imshow_kwargs[i]["vmax"]:.1E}',
                    transform=self.cb_ax[i].transAxes,
                    rotation=90,
            ...
            self.cb_bottom_labels.append(
                self.cb_ax[i].text(
                    1.2,
                    0,
                    f'{self.imshow_kwargs[i]["vmin"]:.1E}',
                    transform=self.cb_ax[i].transAxes,
                    rotation=90,
                    horizontalalignment="left",
        ...
        if self.showCross:
            for i in range(self.n_vols):
                self.l0x.append(
                    self.axes[3 * i + 0].axvline(self.sl_x, color="r", ls=ls)
                )
                self.l0y.append(
                    self.axes[3 * i + 0].axhline(self.sl_y, color="r", ls=ls)
                )

                self.l1x.append(
                    self.axes[3 * i + 1].axvline(self.sl_x, color="r", ls=ls)
                )
                self.l1y.append(
                    self.axes[3 * i + 1].axhline(
                        self.shape[self.iz] - self.sl_z, color="r", ls=ls
                    )
                )

                self.l2x.append(
                    self.axes[3 * i + 2].axvline(self.sl_y, color="r", ls=ls)
                )
                self.l2y.append(
                    self.axes[3 * i + 2].axhline(
                        self.shape[self.iz] - self.sl_z, color="r", ls=ls
                    )
                )
    ...
    def set_vmin(self, i, val):
        if i < self.n_vols:
            ...
            self.imgs[i][0].set_clim(
                [self.imshow_kwargs[i]["vmin"], self.imshow_kwargs[i]["vmax"]]
            )
            self.imgs[i][1].set_clim(
                [self.imshow_kwargs[i]["vmin"], self.imshow_kwargs[i]["vmax"]]
            )
            self.imgs[i][2].set_clim(
                [self.imshow_kwargs[i]["vmin"], self.imshow_kwargs[i]["vmax"]]
            )
    ...
    def redraw_transversal(self):
        for i in range(self.n_vols):
            ...
            if (self.oimgs is not None) and (self.oimgs[i] is not None):
                self.oimgs[i][0].set_data(
                    self.ofac * np.squeeze(self.ovols[i][tuple(self.sl0)].T)
                )
    ...
    def redraw_coronal(self):
        ...
        for i in range(self.n_vols):
            self.imgs[i][1].set_data(
                np.squeeze(np.flip(self.vols[i][tuple(self.sl1)].T, 0))
            )
            if (self.oimgs is not None) and (self.oimgs[i] is not None):
                self.oimgs[i][1].set_data(
                    self.ofac * np.squeeze(np.flip(self.ovols[i][tuple(self.sl1)].T, 0))
                )
        self.ax[0, 1].set_title(str(self.sl_y) + self.fstr, fontsize="small")
        if self.showCross:
            for l in self.l1x:
    ...
    def redraw_sagittal(self):
        for i in range(self.n_vols):
            self.imgs[i][2].set_data(
                np.squeeze(np.flip(self.vols[i][tuple(self.sl2)].T, 0))
            )
            if (self.oimgs is not None) and (self.oimgs[i] is not None):
                self.oimgs[i][2].set_data(
                    self.ofac * np.squeeze(np.flip(self.ovols[i][tuple(self.sl2)].T, 0))
                )
        ...
        if len(self.contour_configs) > 0:
            for cfg in self.contour_configs:
                for i in range(3):
                    ...
                    self.ax[cfg[1], i].contour(
                        self.imgs[cfg[0]][i].get_array(), cfg[2], **cfg[3]
                    )
            self.fig.canvas.draw()
            ...
            if event.inaxes in self.axes:
                ...
                if iax < 3 * self.n_vols:
                    if iax % 3 == 0:
                        ...
                        self.sl_x = int(event.xdata) % self.shape[self.ix]
                        self.sl_z = (
                            self.shape[self.iz] - int(event.ydata)
                        ) % self.shape[self.iz]
                        self.recalculate_slices()
                        self.redraw()
                    elif iax % 3 == 2:
