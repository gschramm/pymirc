...
def convert_contour_data_to_roi_indices(
    contour_data, aff, shape, radius=None, use_contour_orientation=True
):
    ...
    for iroi in range(len(contour_data)):
        ...
        sls = np.array(
            [
                int(round((np.linalg.inv(aff) @ np.concatenate([x[0, :], [1]]))[2]))
        ...
        for sl in sls_uniq:
            ...
            if np.any(np.logical_not(contour_orientations[sl_inds])):
                ...
                for ip in sl_inds:
                    ...
                    i_min = np.floor(
                        (np.linalg.inv(aff) @ np.concatenate([cp.min(axis=0), [1]]))[:2]
                    ).astype(int)
                    i_max = np.ceil(
                        (np.linalg.inv(aff) @ np.concatenate([cp.max(axis=0), [1]]))[:2]
                    ).astype(int)
                    ...
                    for i in np.arange(i_min[0], i_min[0] + n_test[0]):
                        for j in np.arange(i_min[1], i_min[1] + n_test[1]):
                            if poly.contains_point(
                                (aff @ np.array([i, j, sl, 1]))[:2], radius=radius
                            ):
