...
class DicomVolume:
    ...
    def __init__(
        ...
        filelist=None,
        dicomlist=None,
        fallback_series_type=("STATIC", "IMAGE"),
        verbose=True,
    ):
        ...
        if isinstance(filelist, list):
            ...
            self.filelist = glob.glob(filelist)
        else:
            self.filelist = None
        ...
        if self.filelist is not None:
            self.firstdcmheader = dicom.dcmread(self.filelist[0])
        else:
        ...
        # slice dicom files
        if ("NumberOfFrames" in self.firstdcmheader) and (
            self.firstdcmheader.pixel_array.ndim > 2
        ):
            ...
            if "DetectorInformationSequence" in self.firstdcmheader:
                ...
                iop = self.firstdcmheader.DetectorInformationSequence[
                    0
                ...
                try:
                    ...
                    iop = (
                        self.firstdcmheader.SharedFunctionalGroupsSequence[0]
                    ...
                    try:
                        iop = (
                            self.firstdcmheader.PerFrameFunctionalGroupsSequence[0]
                            .PlaneOrientationSequence[0]
                            .ImageOrientationPatient
                        )
                    except AttributeError:
            ...
            self.x = np.array(
                self.firstdcmheader.ImageOrientationPatient[0:3], dtype=float
            )
            self.y = np.array(
                self.firstdcmheader.ImageOrientationPatient[3:], dtype=float
            )
        ...
        if "NumberOfTimeSlices" in self.firstdcmheader:
            self.NumTimeSlices = self.firstdcmheader.NumberOfTimeSlices
        else:
        ...
        if "SeriesType" in self.firstdcmheader:
            ...
            warnings.warn(
                f"Cannot find SeriesType in first dicom header. Setting it to {fallback_series_type}"
            )
    ...
    def get_data(self, frames=None):
        ...
        if not self.read_all_dcms:
            ...
            if self.dicomlist is None:
                self.dicomlist = [dicom.dcmread(x) for x in self.filelist]
            ...
            self.dicomlist = [
                ...
                if not x.SOPClassUID.startswith("1.2.840.10008.5.1.4.1.1.66")
            ]
            ...
            for dcm in self.dicomlist:
                if (dcm.Modality == "MR") and ("AcquisitionNumber" in dcm):
                    ...
                    self.TemporalPositionIdentifiers.append(
                        dcm.TemporalPositionIdentifier
                    )
                else:
            ...
            self.TemporalPositionIdentifiers = np.array(
                self.TemporalPositionIdentifiers
            )
            self.uniq_TemporalPositionIdentifiers = np.unique(
                self.TemporalPositionIdentifiers
            )
            self.uniq_TemporalPositionIdentifiers.sort()

            # if an MR data contains multiple echos, we interpret it as dynamic data
            if (self.dicomlist[0].Modality == "MR") and (
                len(self.uniq_TemporalPositionIdentifiers) > 1
            ):
                self.series_type[0] = "DYNAMIC"
                warnings.warn(
                    f"Found multiple Temporal Positions in MR data set. Setting series type to DYNAMIC"
                )
        ...
        if (self.series_type[0] == "STATIC") or (self.series_type[0] == "WHOLE BODY"):
            ...
            if "NumberOfFrames" in self.firstdcmheader and (
                ...
                if self.filelist is not None:
                    data = self.get_multislice_3d_data(dicom.dcmread(self.filelist[0]))
                else:
        ...
        # read dynamic / gated images
            ...
            if frames is None:
                frames = np.arange(self.nframes) + 1
            ...
            for frame in frames:
                ...
                inds = np.where(
                    self.TemporalPositionIdentifiers
                    == self.uniq_TemporalPositionIdentifiers[frame - 1]
                )[0]
                ...
                if "AcquisitionTime" in self.dicomlist[inds[0]]:
                    self.AcquisitionTimes[frame - 1] = self.dicomlist[
                        inds[0]
                    ].AcquisitionTime
                if "AcquisitionDate" in self.dicomlist[inds[0]]:
                    self.AcquisitionDates[frame - 1] = self.dicomlist[
                        inds[0]
                    ].AcquisitionDate
    ...
    def get_multislice_3d_data(self, dcm_data):
        ...
        dcm_data : pydicom FileDataset
          as returned by pydicom.dcmread
        ...
        if "RescaleSlope" in dcm_data:
            ...
            try:
                ...
                pixelarray = pixelarray * float(
                    dcm_data.SharedFunctionalGroupsSequence[0]
                ...
                pixelarray = pixelarray * float(
                    dcm_data.PerFrameFunctionalGroupsSequence[0]
                    .PixelValueTransformationSequence[0]
                    .RescaleSlope
                )
        ...
        self.v1 = np.array(
            [self.y[0] * self.dr, self.y[1] * self.dr, self.y[2] * self.dr]
        )

        self.v2 = np.array(
            [self.x[0] * self.dc, self.x[1] * self.dc, self.x[2] * self.dc]
        )
        ...
        if ipp is None:
            ...
            warnings.warn(
                "Cannot find ImagePositionPatient in dicom header. Setting it to [0,0,0]"
            )
    ...
    def get_3d_data(self, dicomlist):
        ...
        for i in range(len(pixelarraylistsorted)):
            pixelarraylistsorted[i] = (
                pixelarraylistsorted[i] * RescaleSlopes[i] + RescaleIntercepts[i]
            )
    ...
    def get_3d_overlay_img(self, tag=0x6002):
        ...
        for dcm in dicomlistsorted:
            if [tag, 0x3000] in dcm:
                ...
                tmp = (
                    np.unpackbits(np.frombuffer(overlay, dtype="uint8"))
                    .reshape(-1, 8)[:, ::-1]
                    .flatten()[: (orows * ocols)]
                    .reshape(orows, ocols)
                )
    ...
    #      dicom.dcmwrite(self.filelist[i],dicomlist[i])
...
class DicomSearch:
    ...
    def __init__(self, path, pattern="*.dcm"):
        ...
        for fname in self.allfiles:
            dicomfile = dicom.dcmread(fname, force=True)
        ...
        for uid in self.uniqueUIDs:
            ...
            dicomfile = dicom.dcmread(self.files[-1][0])
            if "SeriesDescription" in dicomfile:
                self.SeriesDescription.append(dicomfile.SeriesDescription)
            else:
                self.SeriesDescription.append(None)
            if "AcquisitionDate" in dicomfile:
                self.AcquisitionDate.append(dicomfile.AcquisitionDate)
            else:
                self.AcquisitionDate.append(None)
            if "AcquisitionTime" in dicomfile:
                self.AcquisitionTime.append(dicomfile.AcquisitionTime)
            else:
                self.AcquisitionTime.append(None)
            if "PatientName" in dicomfile:
                self.PatientName.append(dicomfile.PatientName)
            else:
                self.PatientName.append(None)
            if "Modality" in dicomfile:
                self.Modality.append(dicomfile.Modality)
            else:
                self.Modality.append(None)
