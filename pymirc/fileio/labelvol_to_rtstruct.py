...
def labelvol_to_rtstruct(
    ...
    if isinstance(refdcm_file, list):
        refdcm = pydicom.dcmread(refdcm_file[0])
    else:
        refdcm = pydicom.dcmread(refdcm_file)
    ...
    if roinames is None:
        roinames = ["ROI-" + str(x) for x in roinumbers]
    ...
    for iroi, roinumber in enumerate(roinumbers):
        ...
        for sl in np.arange(z_start, z_end):
            ...
            if dcmVol is None:
                ...
                ds_contour.ReferencedSOPInstanceUID = dcmVol.sorted_SOPInstanceUIDs[
                    sl + sl_offset
                ]
                ds_contour.ReferencedSOPClassUID = dcmVol.sorted_SOPClassUIDs[
                    sl + sl_offset
                ]
            ...
            if bin_slice.max() > 0:
                contours = pymi.binary_2d_image_to_contours(
                ...
                for ic in range(len(contours)):
                    ...
                    for ipoint in range(npoints):
                        contour[ipoint, :] = (
                            aff @ np.concatenate((contours[ic][ipoint, :], [sl, 1]))
                        )[:-1]
