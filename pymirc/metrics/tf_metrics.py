...
def jaccard_index(y_true, y_pred):
    ...
    intersection = K.sum(y_true * y_pred, axis=[1, 2])
    union = K.sum(y_true, axis=[1, 2]) + K.sum(y_pred, axis=[1, 2])
    jaccard = (intersection + K.epsilon()) / (
        union - intersection + K.epsilon()
    )  # this contains as many elements as there are classes
    return jaccard
...
def generalized_dice_coeff(
    ...
    denom = tf.math.reduce_sum(y_true, axis=ax, keepdims=keepdims) + tf.math.reduce_sum(
        y_pred, axis=ax, keepdims=keepdims
    )
    ...
    if reduce_along_features:
        ...
        intersection = tf.math.reduce_sum(
            intersection * feature_weights, axis=-1, keepdims=keepdims
        )
