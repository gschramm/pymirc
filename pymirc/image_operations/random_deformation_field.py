...
def random_deformation_field(shape, shift=2.0, n=30, npad=5, gaussian_std=6):
    ...
    d0 = gaussian_filter(
        np.pad(np.random.randn(n[0], n[1], n[2]), npad[0], mode="constant"),
        gaussian_std[0],
    )
    d1 = gaussian_filter(
        np.pad(np.random.randn(n[0], n[1], n[2]), npad[1], mode="constant"),
        gaussian_std[1],
    )
    d2 = gaussian_filter(
        np.pad(np.random.randn(n[0], n[1], n[2]), npad[2], mode="constant"),
        gaussian_std[2],
    )
    ...
    d0 = d0[
        : min(d0.shape[0], shape[0]),
        : min(d0.shape[1], shape[1]),
        : min(d0.shape[2], shape[2]),
    ]
    d1 = d1[
        : min(d1.shape[0], shape[0]),
        : min(d1.shape[1], shape[1]),
        : min(d1.shape[2], shape[2]),
    ]
    d2 = d2[
        : min(d2.shape[0], shape[0]),
        : min(d2.shape[1], shape[1]),
        : min(d2.shape[2], shape[2]),
    ]
