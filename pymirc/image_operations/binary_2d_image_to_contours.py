...
def cheese_to_contours(img, connect_holes=True):
    ...
    outer_contours = measure.find_contours(
        bin_img_filled == 1, 0.5, positive_orientation="high"
    )
    # get the inner contours (contours around holes)
    inner_contours = measure.find_contours(
        bin_holes == 1, 0.5, positive_orientation="low"
    )
    ...
    if connect_holes:
        ...
        for j in range(len(inner_contours)):
            ...
            dist = np.apply_along_axis(
                lambda x: (x[0] - ip[0]) ** 2 + (x[1] - ip[1]) ** 2, 1, oc
            )
            ...
            closest_outer_points_is_in_contour.append(
                np.any(np.all(np.isin(oc, cop, True), axis=1))
            )

            con = deepcopy(contours[parent_contour_number[j]])

            i_point = np.where(
                np.apply_along_axis(lambda x: np.array_equal(x, cop), 1, con)
            )[0][0]

            contours[parent_contour_number[j]] = np.concatenate(
                [
                    con[: (i_point + 1), :],
                    inner_contours[j],
