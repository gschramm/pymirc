...
def rigid_registration(
    ...
    if downsample_facs is not None:
        for dsf in downsample_facs:
            ...
            vol_fixed_ds = aff_transform(
                vol_fixed, ds_aff, np.ceil(np.array(vol_fixed.shape) / dsf).astype(int)
            )
    ...
    res = minimize(
        ...
        method=method,
        options=opts,
        args=(vol_fixed, vol_float, True, True, metric, pre_affine, metric_kwargs),
    )
    reg_params = res.x.copy()
    ...
    vol_float_coreg = aff_transform(
        vol_float, reg_aff, vol_fixed.shape, cval=vol_float.min()
    )
